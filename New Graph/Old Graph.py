    ...
    for i in range(len(all_types)):
        ...
        for x in numpy.arange(all_types[i].i_restriction, all_types[i].f_restriction + 1, steps):
            try:
                ...
                static.append(create_static(x, float(eval(all_types[i].content) )))
            except: 
                pass
...
def draw_static():      # dont draw in final game (just for testing)
    for ball in static:
        pygame.draw.circle(screen, blue, (ball.body.position[0], ball.body.position[1]), 5)
